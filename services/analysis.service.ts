 ...
 * Includes a self-healing retry policy to fallback to backup models if experiencing high demand.
 */
  ...
  static async analyze(transcript: string, title?: string): Promise<MeetingAnalysis> {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        // Self-healing: if service is unavailable (503), overloaded, or resource exhausted, fallback to backup model and try again
        if (attempts < maxAttempts && (
          ...
          console.warn(`Primary model ${activeModel} overloaded or unavailable. Retrying with backup model ${backupModel}...`);
          activeModel = backupModel;
          
          // Delay briefly to allow rate limits or spike demand to clear
          await new Promise((resolve) => setTimeout(resolve, 1500));
          continue;
        }

        // Break and rethrow if it is a general or non-retryable error
        break;
      }
    }

    // Process last error details if both runs fail
    if (lastError.name === "APITimeoutError" || lastError.message?.includes("timeout") || lastError.message?.includes("APITimeoutError")) {
